...
1. Update the version in `pyproject.toml` (the only place it lives — there is no
   `src/skyward/__init__.py`, the package is PEP 420 namespace-style)
2. Add an entry at the top of `CHANGELOG.md`, and write the fuller notes as
   `docs/releases/vX.Y.Z.md` if the release needs more than a few lines
3. Commit and push
4. Create a GitHub Release with a tag (e.g., `v1.1.0`), using the changelog entry as the
   body
5. The GitHub Action builds and publishes automatically

**A tag alone publishes nothing.** `.github/workflows/publish.yml` triggers on
`release: [published]`, so a version that is tagged but has no Release never reaches
consumers — that is exactly what happened to `v1.6.0`.
...
| `docs/releases/` | Per-version release notes and consumer migration docs (`vX.Y.Z.md`); `CHANGELOG.md` at the repo root is the index |
